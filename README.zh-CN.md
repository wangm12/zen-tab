...
- **实时标签工作区**：在窄侧边栏中查看窗口、原生标签组和未分组标签；顶栏是搜索与设置，底栏切换标签 / 暂存 / 书签。
- **重复标签防护**：URL 一就绪就开始识别重复页，操作快速、可撤销，并隔离普通窗口和隐身窗口。
- **书签 Inbox**：可选书签权限。把未归类条目归档到已有文件夹、预览后再去重，并按标题、网址、文件夹或可选摘要做模糊搜索。
...
- Chrome `sidePanel`、`tabs`、`tabGroups`、`storage` 以及可选的 `bookmarks` API
...
Zen Tab 目前仍处于持续开发中。侧边栏工作区、尽早去重、暂存/恢复、书签 Inbox、AI 分组预览、保守清理、设置、多语言、撤销以及事件驱动更新已经实现。当前支持目标为 Chrome Desktop，暂不承诺 Firefox、Safari 或移动端。
